...
```{r}
south_boston_polls <- poll_location[boston_nhood = "South Boston", ]
```
...
```{r}
south_boston_tree <- tree[boston_nhood = "South Boston", ]
```
...
south_boston_tree <- south_boston_tree %>%
  ...
  mutate(tree_by_walk = !is.na(Name.y))
```


```{r}
n_tree_by_walk <- sum(south_boston_tree$tree_by_walk)
...
```{r}
n_south_boston_tree <- length(south_boston_tree$tree_by_walk)
