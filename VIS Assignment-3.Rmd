...
## 1. A number and proportion of subway stations (point) within a certian distance of free-wifi zone (point)
...
### Calculate the number of subway station within 40 meters of freewifi locations
```{r}
...
About 59% of all subway stations in NYC are within 40 meters of freewifi locations.
...
## 2. Average Distance between the subway stations (point) and free-wifi zone (point)
...
ggplot(com_district) + 
  ...
  scale_color_viridis_c(name = "NYC Subway Stations\nby distance to Free-Wifi",
...
## 3. A number and proportion of Free-Wifi zones (point) in each community districts (polygon) in NYC


### Number of Free-Wifi zones in each community districts
```{r}
com_district <- com_district %>%
...
### Proportion of Free-Wifi zones in each community districts
```{r}
com_district <- com_district %>%
  mutate(num_wifi = lengths(st_covers(com_district, free_wifi))) %>%
  mutate(pct_wifi = num_wifi / free_wifi)
...
## 4. Calculating Average Density of Free-Wifi zones within each Community Districts
...
### Map on the number and proportion of openspaces in each community districts
...
ggplot(open_space) + 
 ...
 scale_fill_viridis_b(name = "NYC Community Districts \nby the Area of Openspace") + 
  annotation_scale(location = "br") + 
  annotation_north_arrow(location = "tr",
                         style = north_arrow_minimal()) +
  theme_map() + 
...
## 7. The Density of subway stations (points) in each district (polygon) in NYC
